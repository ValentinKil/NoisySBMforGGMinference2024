...
mergeUpdate <- function(g, h, ind.all, dataVec, Z, Zmatrix, Q, Rho, theta, threshold){    #on merge g et h et ca devient le groupe g    #Q =Q avant merge
  p=length(Z)
  ...
  k = g; for (l in 1:Qmerge){thetamerge =  updateThetakl_avecRho(ind.all, thetamerge, dataVec, Zmerge, Zmerge_matrix, Rho, k, l)} #updateThetakl_avecRho(thetatest, X, Ztest, Rho, k, l)}
...
mergeUpdate_NIG <- function(g, h, ind.all, dataVec, Z, Zmatrix, Q, Rho, theta, threshold){    #merge g and h and it becomes group g #Q =Q before merge
  p=length(Z)
  ...
  thetamerge$nu=matrix(theta$nu[-ind,],  ncol=2) #otherwise dim pb when only 1 group remains
  thetamerge$w=theta$w[-ind]
  k = g; for (l in 1:Qmerge){thetamerge =  updateThetakl_avecRho_NIG(ind.all, thetamerge, dataVec, Zmerge, Zmerge_matrix, Rho, k, l)} #updateThetakl_avecRho(thetatest, X, Ztest, Rho, k, l)}
